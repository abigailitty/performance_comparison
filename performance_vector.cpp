...
void vector_longest_substring(std::vector<std::string> &vec, std::ostream &ostr, int &output_count) {
  assert(vec.size() >= 2);
  std::string longest = "";
  for(int i = 0; i < int(vec.size()); i++) //word
    for(int j = 0; j < int(vec[i].size()); j++) //all characters in word
      for(int k = i + 1; k < int(vec.size()); k++) //comparison word (all words after current word)
        for(int l = 0; l < int(vec[k].size()); l++) //all character in each comparison word
        {
          if(vec[i][j] == vec[k][l])
          {
            std::string temp = "";
            ...
            while(first_index < int(vec[i].size()) && second_index < int(vec[k].size()) &&
                  vec[i][first_index] == vec[k][second_index])
            {
              temp += vec[i][first_index];
              first_index++;
              second_index++;
            }
